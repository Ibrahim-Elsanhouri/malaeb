<?php namespace App\Http\Controllers\CrudBooster\Controllers;

use App\Http\Controllers\CrudBooster\Controllers\Controller;
...
class AdminController extends CBController {	
	...
	public function postLogin() {		
		...
		$users 		= DB::table(config('crudbooster.USER_TABLE'))->where("mobile",$mobile)->first(); 		
		...
		if(\Hash::check($password,$users->password)) {
			...
			$photo = ($users->photo)?asset($users->photo):'https://www.gravatar.com/avatar/'.md5($users->mobile).'?s=100';
