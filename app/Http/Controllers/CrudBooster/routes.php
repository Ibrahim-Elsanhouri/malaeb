...
$namespace = 'App\Http\Controllers\CrudBooster\Controllers';
...
Route::group(['middleware'=>['web','\crocodicstudio\crudbooster\middlewares\CBBackend'],'prefix'=>config('crudbooster.ADMIN_PATH'),'namespace'=>$namespace], function () {
	...
	CRUDBooster::routeController('/','AdminController',$namespace='App\Http\Controllers\CrudBooster\Controllers');	
	CRUDBooster::routeController('api_generator','ApiCustomController',$namespace='App\Http\Controllers\CrudBooster\Controllers');	
	...
	try{
		...
		foreach($moduls as $v) {
			if(@$v->path && @$v->controller) {		
				CRUDBooster::routeController($v->path,$v->controller,$namespace='App\Http\Controllers\CrudBooster\Controllers');
			}
		}
	}catch(Exception $e) {
